...
COPY backend/ ./
# В /app/frontend уезжает только результат сборки Vite. Ни .env, ни исходники
# сюда не попадают: этот каталог отдаётся наружу целиком, и всё, что в нём
# лежит, публично. Секреты приходят в контейнер переменными окружения.
COPY --from=frontend /build/dist /app/frontend

# Приложение не должно работать от root даже внутри контейнера.
RUN useradd --system --uid 10001 litclub && chown -R litclub:litclub /app
USER litclub
